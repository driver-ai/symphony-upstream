defmodule SymphonyElixir.AppServerTest do
  ...
  test "run_turn interrupts the active turn and reports the stop request" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-graceful-stop-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      codex_binary = Path.join(test_root, "fake-codex")
      interrupt_log = Path.join(test_root, "interrupt.log")
      File.mkdir_p!(workspace)

      # The fake never completes the turn on its own: only a turn/interrupt ends it, with the
      # interrupted status a real app-server reports.
      File.write!(codex_binary, """
      #!/bin/sh
      count=0
      while IFS= read -r line; do
          ...
          *)
            case "$line" in
              *turn/interrupt*)
                printf '%s\\n' "$line" > "#{interrupt_log}"
                printf '%s\\n' '{"id":4,"result":{}}'
                printf '%s\\n' '{"method":"turn/completed","params":{"turn":{"id":"turn-stop","status":"interrupted"}}}'
                exit 0
                ;;
            esac
            ;;
      ...
      issue = %Issue{
        id: "issue-graceful-stop",
        identifier: "MT-STOP",
        title: "Stop gracefully",
        description: "Ensure a stop request interrupts the turn instead of killing the run",
        state: "In Progress",
        url: "https://example.org/issues/MT-STOP",
        labels: ["backend"]
      }

      task = Task.async(fn -> AppServer.run(workspace, "keep working", issue) end)
      Process.sleep(300)
      assert :ok = SymphonyElixir.AgentRunner.request_stop(task.pid, :issue_inactive)

      assert {:ok, %{result: :stopped, stopped: :issue_inactive, thread_id: "thread-stop", turn_id: "turn-stop"}} =
               Task.await(task, 5_000)

      interrupt = File.read!(interrupt_log)
      assert interrupt =~ "turn/interrupt"
      assert interrupt =~ "thread-stop"
      assert interrupt =~ "turn-stop"
    after
      File.rm_rf(test_root)
    end
  end
